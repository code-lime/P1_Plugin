...
public final class VanillaReplaceComponent extends ComponentStatic<JsonObject> {
    private static boolean is(List<Toast2<String, List<String>>> filter, Map<String, String> values) {
        for (Toast2<String, List<String>> kv : filter) {
    ...
    public VanillaReplaceComponent(BlockInfo creator, JsonObject json) {
        ...
        json.entrySet().forEach(kv -> {
            ...
            List<Toast2<String, List<String>>> filter = new ArrayList<>();
            for (String _arg : _args) {
                ...
                filter.add(Toast.of(_kv[0], Arrays.asList(Arrays.stream(_kv).skip(1).collect(Collectors.joining("=")).split(","))));
            }
