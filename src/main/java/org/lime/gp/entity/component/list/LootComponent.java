...
public final class LootComponent extends ComponentStatic<JsonObject> implements CustomEntityMetadata.Lootable {
    public final Map<String, Integer> items;
    public final Map<String, String> args = new HashMap<>();

    public LootComponent(EntityInfo info, JsonObject json) {
        super(info, json);
    ...
    public LootComponent(EntityInfo info, List<Material> items) {
        super(info);
        this.items = items.stream().collect(Collectors.toMap(Enum::name, v -> 1));
    }

    public LootComponent(EntityInfo info, Map<Material, Integer> items) {
        super(info);
