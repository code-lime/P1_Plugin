...
public class ViewPosition {
    ...
    private ViewPosition(Toast2<Integer, Integer> point, Toast2<Double, Double> pointD, Position block, MapMonitor.ClickType click) {
        this(point.val0, point.val1, pointD.val0, pointD.val1, block, click);
    }
    private ViewPosition(Toast2<Toast2<Integer, Integer>, Toast2<Double, Double>> point, Position block, MapMonitor.ClickType click) {
    ...
    private static Toast2<Double, Double> rotate(Double x, Double y, MapMonitor.MapRotation rotation) {
        return switch (rotation) {
            default -> Toast.of(x, y);
            case CLOCKWISE -> Toast.of(y, 1-x);
            case FLIPPED -> Toast.of(1-x, 1-y);
            case COUNTER_CLOCKWISE -> Toast.of(1-y, x);
        };
    }
    private static Optional<Toast2<Toast2<Integer, Integer>, Toast2<Double, Double>>> point(MonitorInstance meta, MapMonitor.MapRotation rotation, Player player) {
        return pointD(meta, rotation, player)
                .map(v -> Toast.of(Toast.of((int)Math.round(v.val0 * 127), (int)Math.round(v.val1 * 127)), v));
    }
    private static final double UP_OFFSET = ((1+(8/60.0))/16.0);
    private static Optional<Toast2<Double, Double>> pointD(MonitorInstance meta, MapMonitor.MapRotation rotation, Player player) {
