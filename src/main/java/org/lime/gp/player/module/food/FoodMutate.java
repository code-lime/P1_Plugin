...
public class FoodMutate {
    ...
    public void tick(EntityHuman player) {
        ...
        if (this.exhaustionLevel > 4.0f) {
            ...
            if (this.saturationLevel > 0.0f) {
                ...
                ((EntityPlayer) player).connection.send(new PacketPlayOutUpdateHealth(((EntityPlayer) player).getBukkitEntity().getScaledHealth(), (int) this.food.totalLevel(), this.saturationLevel));
            }
        }
    ...
    public void readAdditionalSaveData(NBTTagCompound nbt) {
        if (!nbt.contains("foodLevel", 99)) return;
