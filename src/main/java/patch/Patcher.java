...
public class Patcher {
    ...
    private static void throwablePatch(byte[] resource) throws Throwable {
        ...
        Native.log("Patch version: " + patch_version);
        ...
        try (Closeable ignored = Native.loadDeobf()) {
            ...
            version_archive
                    ...
                    .patchMethod(IMethodFilter.of(TileEntityTypes.class, STATIC_CONSTRUCTOR, false),
                            MethodPatcher.mutate(v -> new MethodVisitor(Opcodes.ASM9, v) {
                                private final String from = TileEntitySkull.class.getName().replace('.', '/');
                                private final String to = from.replace("TileEntitySkull", "TileEntityLimeSkull");
                                @Override public void visitInvokeDynamicInsn(String name, String descriptor, Handle handle, Object... args) {
                                    super.visitInvokeDynamicInsn(name, descriptor, handle, Arrays.stream(args).map(v -> {
                                        if (v instanceof Handle _handle && _handle.getOwner().equals(from)) {
                                            Native.log("Replace '" + from + "' to '" + to + "' in " + v);
                                            return new Handle(_handle.getTag(), to, _handle.getName(), _handle.getDesc(), _handle.isInterface());
                                        }
                                            ...
                                            if (method.getReturnType().equals(Type.getType(TileEntitySkull.class))) {
                                                Native.log("Replace '" + from + "' to '" + to + "' in " + v);
                                                return Type.getMethodType(Native.replaceDescriptor(Type.getType(TileEntitySkull.class), "TileEntitySkull", "TileEntityLimeSkull"), method.getArgumentTypes());
                                            }
                                        }
                                        return v;
                                    }).toArray());
                    ...
                    .patchMethod(IMethodFilter.of(EntityTypes.class, STATIC_CONSTRUCTOR, false),
                            MethodPatcher.mutate(v -> new MethodVisitor(Opcodes.ASM9, v) {
                                private final String from = Marker.class.getName().replace('.', '/');
                                private final String to = from.replace("Marker", "EntityLimeMarker");
                                    ...
                                    super.visitInvokeDynamicInsn(name, descriptor, handle, Arrays.stream(args).map(v -> {
                                        if (v instanceof Handle _handle && _handle.getOwner().equals(from)) {
                                            ...
                                            if (method.getReturnType().equals(Type.getType(Marker.class))) {
                                                Native.log("Replace '" + from + "' to '" + to + "' in " + v);
                                                return Type.getMethodType(Native.replaceDescriptor(Type.getType(Marker.class), "Marker", "EntityLimeMarker"), method.getArgumentTypes());
                                            }
                                        }
                                        return v;
                                    }).toArray());
            ...
            version_archive
                    ...
                    .patchMethod(IMethodFilter.of(Items.class, STATIC_CONSTRUCTOR, false),
                            MethodPatcher.mutate(v -> new MethodVisitor(Opcodes.ASM9, v) {
                                private final String from = InstrumentItem.class.getName().replace('.', '/');
                                private final String to = from.replace("InstrumentItem", "InstrumentSoundItem");
